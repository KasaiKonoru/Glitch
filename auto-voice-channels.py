...
async def update_status(client):
    if client.is_ready():
        ...
        if guilds:
            ...
            nc = utils.num_active_channels(guilds)
            text = "({}) Tatsuka Channel".format(nc, ("s" if nc != 1 else ""))
        else:
            text = "🚧Nothing🚧"
        ...
        if text != old_text:
            try:
                await client.change_presence(activity=discord.Activity(name=text, type=discord.ActivityType.listening))
