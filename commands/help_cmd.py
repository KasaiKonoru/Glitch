...
async def execute(ctx, params):
    ...
    if not params:
        support_server_id = 433980111558279185
        if not ctx['admin'] and ctx['guild'].id != support_server_id:
            e = discord.Embed(color=discord.Color.from_rgb(205, 220, 57))
            e.title = "VoiceCreate"
            e.description = (
                "I'm a bot that allows your users to dynamically and infinitely create voice channels as they need them, "
        ...
        for i, s in enumerate(sections):
            s = s.replace("@VoiceCreate ", "@{} ".format(ctx['message'].guild.me.display_name))
            s = s.replace("vc/", esc_md(ctx['prefix_p']))
            s = s.replace("@Tatsuka KasEI", author.mention)
        ...
        if c in commands:
            ...
            for part in help_text:
                ...
                if part == help_text[-1]:
                    e.set_footer(text="More help: https://discordapp.com/invite/bppKsVz",
